...
public class BigFileOrBreakPointUploadUtilWithHttps {
	
	

	/* 上传文件至Server的方法 */
	public static  PrepareFileEntity uploadBigFileMethod(String fuid, String token,
			String uploadFile, long size, long startPosition, int pid, int aid,int sizeK,Context context) {
		PrepareFileEntity entity2return = null;
		ShareUtils util = new ShareUtils(context);
		String actionUrl = util.getURL()+"/a1/resumeUpload?";
	/*	//https用
...
//					}
				...
				if(pb!=null)
				pb.setProgress((int)(start/size*100));
				PrepareFileEntity entity =  BigFileOrBreakPointUploadUtilWithHttps.uploadBigFileMethod(fuid,
						token, file, file2.length(),
